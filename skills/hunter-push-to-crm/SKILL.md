...
- `List-Connected-Apps` to show integrations. Paginate with `offset`/`limit` (default page size 20) until every row is read (`meta.total` / `offset`). Then keep only lead-syncing CRM providers (`HubSpot`, `Pipedrive`, `Salesforce`, `Zapier`, `Zoho`). Ignore Google Sheets, SMTP/IMAP inboxes, and other non-CRM apps — `Push-Leads-To-CRM` rejects them. `Get-Connected-App` for details on one eligible target.
- If the **fully paged** filtered list is empty, tell the user to connect a CRM in Hunter first (Settings → Integrations) — this skill can't create the connection. Do not declare “no CRM” from the first page alone.
