...
Save companies and organize them into lists and folders — the account-level counterpart to `hunter-lead-lists`.
...
- Companies typically come from `hunter-discover` (`Find-Companies`) or `hunter-company-enrichment`.
...
- Note: list membership and bulk **move** require static lists; bulk **copy** requires only the *destination* static (its source may be dynamic). `Bulk-Delete-Companies` accepts any team-owned list, including dynamic ones. Dynamic (auto-filter) lists can't be the target of add/move — pick a static target when the user chooses from `List-Company-Lists`.
...
## Next Actions
1. Find contacts at these companies (`hunter-domain-search`)
2. Save decision-makers as leads (`hunter-manage-leads` / `hunter-lead-lists`)
3. Run a full prospecting pass on the list (`hunter-prospecting`)
```
...
- Company lists organize companies (accounts); use `hunter-lead-lists` for contacts.
