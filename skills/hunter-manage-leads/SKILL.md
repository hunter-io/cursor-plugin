...
- **Add a lead** → `Create-Lead-If-Missing` to add without touching an existing lead's data, or `Create-Or-Update-Lead` when the user intends to update in place (it overwrites existing fields). `Create-Lead` creates only when the email is not already a lead; if it is, the API returns 422 `duplicated_entry` — it cannot force a second record.
...
Include every supported present field when creating: `email`, `first_name`, `last_name`, `position`, `company`, `company_industry`, `company_size`, `company_type`, `website`, `country_code`, `linkedin_url`, `phone_number`, `twitter`, `notes`, `source`. Custom-attribute *values* still cannot be set through the lead tools (see Custom attributes below).
...
## Next Actions
1. Add her to a leads list (`hunter-lead-lists`)
2. Add her to an outreach sequence (`hunter-build-sequences`)
3. Push to your CRM (`hunter-push-to-crm`)
```
