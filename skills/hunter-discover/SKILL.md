...
Find companies matching any criteria and size how many contacts they hold — all free, no credits consumed. To turn companies into named, verified contacts you then use Domain Search (which consumes credits); for the full "find people by role" pipeline, use the `hunter-prospecting` skill.
...
## Next Actions
1. Narrow or refine the criteria to surface different results (`Find-Companies` has no offset paging; `Find-People` accepts `limit`/`offset` — page when more than 100 companies match)
...
4. Save companies to a list (`hunter-company-lists`)
5. Run the full people pipeline on these (`hunter-prospecting`)
6. Rerun one of your saved searches (List-Saved-Searches)
```

3. **If the tool returns an `error` envelope** (auth, quota, rate limit, validation, 5xx), surface that error. Do not say no companies matched.

4. **If results are too broad** (hundreds of companies), suggest narrowing: "That's a broad search. Try adding filters like industry, size, location, or technology."

5. **If the call succeeded and zero companies were returned,** suggest loosening criteria: "No companies matched. Try broadening — for example, widen the location or employee range."

6. **Remind users this is free:** "Discovering companies is free — refine as many times as you'd like. You only spend credits when you pull the actual contacts with Domain Search."
...
At least one company returned matching the user's criteria — with people-by-role requests correctly routed to Domain Search / `hunter-prospecting` rather than to Find-People.
